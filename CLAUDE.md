...
설정 파일을 고쳤으면 **작업 도중에** 서버를 재시작한다. 어떤 파일이 해당하는지는
아래 [재시작이 필요한 변경](#재시작이-필요한-변경-필수) 을 참고한다.
재시작하지 않으면 코드가 맞아도 화면에 반영되지 않아, 멀쩡한 코드를 잘못됐다고 오판하게 된다.
...
아래 파일을 수정했으면 **작업자에게 묻지 말고 즉시 서버를 재시작한다.**
설정 파일은 서버 기동 시점에만 읽히므로, 재시작하지 않으면 코드가 맞아도 화면에 반영되지 않는다.
