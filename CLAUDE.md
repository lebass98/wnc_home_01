...
코드 작업이 끝나면 커밋하기 전에, 이번 작업이 프론트(홈페이지)·관리자·DB
세 층에 제대로 이어져 있는지 확인한다. 한쪽에만 존재하는 "따로 노는" 기능을
만들지 않는다.

**프론트 ↔ 관리자**

- 관리자에서 값을 바꾸면 → 홈페이지 화면에 실제로 반영되는지
- 프론트에 보이는 데이터를 만들었으면 → 관리자에 그것을 관리할 화면이 있는지
- API 를 추가·변경했으면 → `apps/web/src/lib/demoApi.ts` 에도 같은 동작을 넣었는지
- 확인은 실제 브라우저로 한다: 관리자에서 조작 → 홈페이지에서 결과를 눈으로 확인
...
- `seed.ts` 를 바꿨으면 → 시드는 표가 비어 있을 때만 돌기 때문에
  (`count() === 0` 가드) **기존 로컬 DB 에는 반영되지 않는다.**
  새 구조·데이터가 로컬 DB 에도 들어갔는지 확인하고, 안 들어갔으면
  직접 반영하거나 사용자에게 알린다. 메뉴·글 분류가 옛 구조로 남아
  화면이 비어 보였던 사고가 실제로 있었다.

어긋난 곳을 찾으면 **고친 뒤에** 커밋한다. 이번 작업 범위를 벗어나는 문제라면
커밋은 하되 사용자에게 어긋난 내용을 알린다.
