...
- 오늘 날짜 항목이 이미 있으면 **그 날짜의 맨 위에** 줄을 덧붙이고, 없으면 `### YYYY-MM-DD` 를 새로 만든다.
- 최신 날짜가 위로, 같은 날짜 안에서도 최신 작업이 위로 오도록 정렬한다.
- **작업 하나당 정확히 한 줄만 쓴다. 하위 항목(들여쓴 `-`)을 만들지 않는다.**
  자세한 내용은 커밋 메시지에 적는다. 일지는 훑어보는 용도다.
- 한 줄이 길어지면 `—` 뒤에 핵심만 덧붙인다. 파일명은 나열하지 않는다.
  - 좋음: `제품 목록에 가격 필터 추가`
  - 좋음: `팝업 관리 기능 신설 — 게시기간·노출위치 설정, 진행 상태 필터`
  - 나쁨: `ProductsPage.tsx, api.ts 수정`
  - 나쁨: 항목 아래에 세부 내용을 여러 줄로 나열하는 것

일지가 길어져 훑어보기 어려워지면 지난 항목을 한 줄로 줄여 정리한다.
