...
- **메인 화면 전면 개편** — 참고 템플릿(THEME039) 구성을 그대로 가져와 다시 만듦
  - 자동 전환 히어로 배너, 히어로 위에 투명하게 얹히고 스크롤하면 흰 배경으로 바뀌는 헤더
  - 소개 2단 문단, 넓은 영상 띠, 등록된 제품을 보여주는 프로젝트 캐러셀(어두운 띠 위)
  - 번호가 붙은 개발 철학 4칸, 서비스 카드 3종, 민트 블록이 어긋나게 깔린 영상 구역, 공지 2단
  - 강조색으로 민트(#7dbbbd) 계열을 추가

- **회사소개 페이지** 개편 — 참고 템플릿의 소개 화면 구성으로 다시 만듦
  - 왼쪽 제목 + 오른쪽 본문 2단, 이미지 2장, 영상 구역, 서비스 철학 2단, 강점 카드 3종
- 서브 페이지 상단 배너를 어두운 형태로 바꾸고 하위 탭을 붙임 (회사소개·사업분야 등 6개 화면 공통)
- 로고를 영문 **WORDNCODE** 로 바꾸고 앞의 W 아이콘을 삭제 (헤더·푸터)

- **스크롤 인터랙션** 추가 — 화면에 들어오는 요소가 아래에서 위로 순차적으로 나타남
  - 메인·회사소개 전 구역과 공통 요소(구역 제목, 서브 페이지 배너, 카드 목록)에 적용
  - 브라우저에 '움직임 줄이기'가 켜져 있으면 애니메이션 없이 바로 표시

- **홈페이지 팝업 표시 방식 개편** — 화면 가운데에 겹쳐 띄우는 형태로 변경
  - 2장까지는 한 화면에 나란히, 3장부터는 좌우로 넘겨서 봄 (2장 이하면 화살표·점 표시 없음)
  - 아래에 '팝업건수 : 총 N건', '닫기', '오늘 하루 열지 않기' 버튼
  - 팝업이 떠 있는 동안 뒤쪽 화면 스크롤 잠금, ESC 로 닫기

- **KRDS 형태 날짜 선택기** 도입 — 브라우저마다 다르게 보이던 기본 달력을 직접 만든 달력으로 교체
  - 단일 선택과 시작~종료 기간 선택 두 가지, 기간은 달력 하나에서 양 끝과 사이 구간을 이어서 표시
  - 연·월 바로 선택, 오늘 표시(붉은 점), 일요일 강조, 시각(시·분) 선택, 직접 타자 입력 지원
  - 팝업 관리의 게시기간 검색과 등록·수정 화면에 적용
  - 기간 표시를 참고 디자인에 맞춰 칸을 꽉 채우는 띠로 바꾸고, [확인] 을 눌러야 값이 반영되도록 수정

- **팝업 관리** 기능 신설 — 관리자에서 홈페이지 팝업을 만들고 게시기간·노출을 관리
  - 목록: 진행 상태(진행대기·진행중·종료·중지) 필터, 게시기간 검색(오늘/이번주/이번달), 이름 검색, 사용여부 토글, 선택 삭제
  - 등록·수정: 팝업위치(메인/특정페이지), 게시기간, 창 크기·위치, 링크(현재창/새창), 스크롤바, 창 형태(일반 윈도우·고정 레이어·이동 가능한 레이어), 표시기간, 이미지·본문 편집기
  - 홈페이지: 지정한 화면에서만 노출되고, '다시 보지 않기'를 고르면 설정한 기간만큼 감춰짐
  - 검색 영역의 검색·초기화 버튼 크기를 서로 맞춤

- 어드민 **다국어(i18n) 기능 구조** 구현 — 한국어·English·日本語·中文 4개 언어팩
  - 상단 언어 선택 드롭다운, 브라우저 언어 자동 감지, 선택값 저장
  - 사이드바·로그인·게시판 목록·게시판 설정 화면에 언어팩 연결
  - 번역문은 아직 한국어 원문 상태이며, `apps/web/src/locales/*.json` 값만 채우면 반영됨
