...
import PageHero, { type PageHeroTab } from './PageHero'
...
/**
 * 서브 화면 공통 틀 — 상단 히어로 아래에 본문을 놓는다.
 * 페이지 관리에서 이 경로를 '좌측 메뉴 서브'로 골랐으면
 * 현재 화면이 속한 메뉴 묶음을 왼쪽에 세우고 본문을 오른쪽에 둔다.
 */
...
export default function SubPage({
  ...
  title: string
  description?: string
  /** 히어로 아래 작은 탭 — 기본 서브에서만 보인다. 좌측 메뉴 서브는 왼쪽 메뉴가 그 역할을 한다. */
  tabs?: PageHeroTab[]
  children: ReactNode
}) {
  ...
  const layout = layoutFor(pathname, layouts)
  // 좌측 메뉴에는 사이트맵 노출 기준을 쓴다 — GNB 에 없는 묶음(이용안내 등)도 세울 수 있다.
  const group = layout === 'left' ? findGroup(pickMenu(siteMenu, 'sitemap'), pathname) : null

  // 좌측 메뉴로 쓸 묶음이 없으면 기본 서브로 그린다.
  if (layout !== 'left' || !group || group.children.length === 0) {
    return (
      <>
    ...
    <>
      ...
      <div className="container-wnc py-14 sm:py-16">
        <div className="grid gap-12 gnb:grid-cols-[15rem_minmax(0,1fr)]">
          ...
          {/*
            본문 — 페이지들이 쓰는 container-wnc 는 폭 제한과 좌우 여백만 주므로
            좁은 칼럼 안에서는 칼럼 폭에 맞아 그대로 동작한다.
          */}
