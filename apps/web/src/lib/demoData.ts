import boardSamples from '../../../../packages/shared/src/boardSamples.json'
import type { Branch, Contact, ContactStatus, MenuAutoChildren, Post } from '@wnc/shared'
...
export function createDemoPosts(): DemoPost[] {
  return boardSamples.map(({ category, title, content, thumbnail }, i) => ({
    id: i + 1,
    category,
    title,
    content,
    thumbnail: thumbnail ? `${import.meta.env.BASE_URL}${thumbnail.slice(1)}` : null,
